/* ------------------------------------------------------------------------
	...
	EDITED: 4-15-2021 (umer936)
	    - replaced .size()   -> .length to be jQuery 3 compatible
	    - replaced .bind()   -> .on() to be jQuery future compatible
	    - replaced .unbind() -> .off() to be jQuery future compatible
	    - replaced .click()  -> .on('click') to be jQuery future compatible
------------------------------------------------------------------------- */
(function($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		...
		*/
		$.prettyPhoto.initialize = function() {
			...
			pp_titles = (isSet) ? jQuery.map(matchedObjects, function(n, i){ if($(n).attr(settings.hook).indexOf(theRel) != -1) return ($(n).find('img').attr('alt')) ? $('<div/>').text($(n).find('img').attr('alt')).html() : ""; }) : $.makeArray($('<div/>').text($(this).find('img').attr('alt')).html());
		...
		*/
		$.prettyPhoto.open = function(event) {
			...
			$pp_pic_holder.fadeIn(function(){
				// Set the title
				(settings.show_title && pp_titles[set_position] != "" && typeof pp_titles[set_position] != "undefined") ? $ppt.text(pp_titles[set_position]) : $ppt.html('&nbsp;');
